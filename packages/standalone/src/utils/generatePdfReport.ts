...
async function loadScreenshots(_results: AnalysisResult[]): Promise<Map<string, string>> {
  // URL screenshots are a desktop-only feature (Electron captures them), so
  // standalone results never carry a screenshotPath to load. Kept as a
  // function so this file stays a near-verbatim copy of the desktop
  // renderer's report generator.
  return new Map();
}
